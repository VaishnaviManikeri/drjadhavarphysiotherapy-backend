...
router.post('/appointments', async (req, res) => {
  try {
    ...
    console.log('Received appointment request:', { name, mobile, problem, preferredDate, preferredTime });
    ...
    // 2. Auto-reply to Patient (via email if available, or we'll send via WhatsApp)
    // Note: Since we only have mobile, we'll rely on WhatsApp for auto-reply
    // But we'll still try to send email if they provided one (we need to add email field)
